...
async function broadcastState(roomCode: string, state: GameState) {
    // Save but never allow it to prevent updates
    saveGame(state).catch((e) =>
        console.warn("[broadcastState] saveGame failed:", e?.message ?? e)
    );
...
io.on("connection", (socket) => {
    ...
    socket.on("turn:roll", async ({ chosenDeckIfFive }: { chosenDeckIfFive?: "FIN" | "SITE" | "UPG" | "PROD" }) => {
        const ctx = ctxBySocket.get(socket.id)!;
        if (!ctx.roomCode || !ctx.playerId) return;
    ...
    socket.on("turn:apply", async (payload: {
        coinsSpent: 0 | 1 | 2;
        chosenCardId?: string;
        ...
        try {
            applyTurn(state, ctx.playerId, payload.coinsSpent, payload.chosenCardId, payload.chosenDeckIfFive);
            await broadcastState(ctx.roomCode, state);
        } catch (e: any) {
            socket.emit("error:msg", { message: e.message ?? "Apply failed" });
        }
    });

    socket.on("disconnect", () => ctxBySocket.delete(socket.id));
});
